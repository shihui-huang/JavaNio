...
import tsp.csc4509.dm.rpc.RpcType;
...
public class AppliRpcClient {
        ...
        /**
         * position du port du id du rpctype dans argv[].
         */
        private static final int IDARG = 2;
         /**
          * position du port du type du param dans argv[].
          */
    private static final int PARAMARG = 3;
    ...
    public static void main(final String[] argv) throws IOException, ClassNotFoundException {
        ...
        if (argv.length < 3) {
            System.out.println("usage: java AppliRpcClient <machine> <port> <id> (<param> <val1> <val2> (<val3> <val4>))");
            System.out.println("id: LIST/AVERAGELOAD/COMPUTELIST/COMPUTE");
            System.out.println("Param only exists when id is COMPUTE。 param: add/multipli. add: val1 + val2. multipli: (val1 + i val2) * (val3 + i val4)");
            return;
        }

        String serverHost = argv[HOSTARG];
        int serverPort = Integer.parseInt(argv[PORTARG]);
        String id = argv[IDARG];
        ...
        if(id.equals("COMPUTE")){
            ...
            switch (param) {
                case "add" :
                    ...
                    RpcClient rpcClientAdd = new RpcClient(serverHost, serverPort, "COMPUTE", (Class<? extends RpcParam>) additionParam.getResultClass(), additionParam);
                ...
                case "multipli":
                    ...
                    RpcClient rpcClientMultipli = new RpcClient(serverHost, serverPort, "COMPUTE", (Class<? extends RpcParam>) multipliParam.getResultClass(), multipliParam);
            ...
            Serializable result = rpcClient.getResult();
        }
    }
}
